...
normal_multi_x *mean_normal_x(double **X, int num_train, int num_feat)
{
	...
	result = calloc(1, sizeof(normal_multi_x));
...
{
	int i;
	normal_multi_x *result_X = NULL;
	normal_single_y *result_y = NULL;
	...
	result_X = mean_normal_x(data_set->X, data_set->num_train, data_set->num_feat);

	result_y = mean_normal_y(data_set->y, data_set->num_train);

	write_to_file(result_X->X, result_y->y, data_set->num_train, data_set->num_feat,
		      output_file);

	for (i = 0; i < data_set->num_train; i++) {
